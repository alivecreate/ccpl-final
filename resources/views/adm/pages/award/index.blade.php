...
<div class="content-wrapper">
  <section class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1>Award Recognization List</h1>
        </div>
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            ...
            <a href="{{route('award.create')}}" class="btn btn-success btn-sm ml-2"><i class="fa fa-plus" aria-hidden="true"></i>
              &nbsp;&nbsp;Add Award Recognization </a>
          </ol>
        </div>
      </div>
    </div>
        ...
        <div class="col-12">
          <div class="card">
              ...
              <form action="{{route('item.bulk-delete')}}" method="post">
                ...
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tbody>
                    ...
                    <tr>
                      <td>
                        <input type="checkbox" class="checkList" name="checkList[]" id="checkList" value="{{$award->id}}" />
                      </td>

                      <td>{{++$i}}</td>
                      @if(isset(getMedia($award->id, 1 , 'award' ,'asc')->image))
                      <td class="text-center"><img class="img-rounded " height="80" src="{{asset('web')}}/media/xs/{{getMedia($award->id, 1 , 'award' ,'asc')->image}}"></td>
                      @else
                      ...
                      <td>
                        <a href="{{route('admin.manage-photo',$award->id)}}">Image</a>
                      </td>
                      <td>
                        <div class="form-check">
                          <input type="checkbox" class="form-check-input  pull-right" name="status" id="exampleCheck1" onClick="updateStatus({{$award->id}})" @if($award->status == 1)checked
                          @endif
            ...
            <td>
              ...
              <a target="_blank" href="{{url('')}}/{{$finalSlug}}{{$award->slug}}" class="btn btn-xs btn-warning float-left mr-2" title="Upload Award Recognization Images">
                <i class="fa fa-eye"></i></a>

              <a href="{{route('award.edit', $award->id)}}" class="btn btn-xs btn-info float-left mr-2" title="Edit Award Recognization"><i class="far fa-edit"></i></a>



              <button type="button" class="btn btn-xs btn-danger del-modal float-left" title="Delete Award Recognization" data-id="{{route('award.destroy', $award->id)}}" data-title="{{ $award->name}}" data-toggle="modal" data-target="#modal-default"><i class="fas fa-trash-alt"></i>
              </button>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title">Delete Award recognization</h4>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <label>Award recognization Name</label>
        <h5 class="modal-title delete-title">Delete Award recognization</h5>
      </div>
