...
<div class="parallax-layout parallax overlay header-video">
...
<section class="p-4 portfolio-section portfolio-padding grid-portfolio ratio2_3">
    <div class="container">
            ...
            @foreach($videos as $video)
                <button class="filter-button project_button" data-filter="{{$video->id}}">{{$video->name}}</button>  
                @endforeach
            </div>
           
            <div class="row  zoom-gallery">
            @foreach($videos as $video)
                @foreach(getMedia($video->id, 0 , 'video' ,'asc') as $image)
                    <div class="isotopeSelector filter {{$video->id}} col-lg-4 col-sm-6 btn-inquiry">
                        <div class="overlay">
                    ...
                    <img src="{{url('')}}/web/media/md/{{$image->image}}" height="130"  style="max-height:200px"/>
                        @else
                                ...
                                <a data-fancybox href="{{getYoutubeThumb($image->youtube_embed)['url']}}&amp;rel=0&amp;controls=0&amp;showinfo=0">
                                    <div class="overlay-background">
                                        <i class="fa fa-youtube-play" aria-hidden="true" 
                                        style="font-size:80px;color:red"></i>
                                    </div>
                                    <img src="https://img.youtube.com/vi/{{getYoutubeThumb($image->youtube_embed)['thumb']}}/hqdefault.jpg"
