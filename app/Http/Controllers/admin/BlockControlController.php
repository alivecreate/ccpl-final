...
class BlockControlController extends Controller
{
    ...
    public function pageLinkCreate(Request $request)
    {
        if($request->type == 'edit'){
            ...
            if(!$pageLink){
              return redirect(route('pageLink.create'));  
            }else{
            $data = [
                'pageLink' =>  $pageLink,
    ...
    if(DB::table($table)->where('id', $id)->first()){
        ...
        if($data_slug == 'page'){
            return redirect(route('pageLink.create'))->with('success', 'Deleted Successfully...');
        }
        elseif($data_slug == 'url_list' || $data_slug == 'product' ||  $data_slug == 'category'
                || $data_slug == 'testimonial' || $data_slug == 'blog'){

            return redirect(route('commonLink.create',$request->data_slug))->with('success', 'Deleted Successfully...');
        }
