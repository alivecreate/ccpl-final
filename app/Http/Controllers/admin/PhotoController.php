...
class PhotoController extends Controller
{
    ...
    public function Index(Request $request)
    {
        ...
        if ($request->page == 'manage' && isset($request->item_id)) {
            $isProduct = DB::table('products')->where('id', $request->item_id)->first();

            $photoDetails = DB::table('media')->where('media_id', $isProduct->id)->orderBy('item_no', 'ASC')->get();
            ...
            if ($isProductAvailable) {
                $photoDetails = DB::table('media')->where('media_id', $isProductAvailable->id)->orderBy('item_no', 'ASC')->get();
            } else {
                $photoDetails = null;
            }
            $data = [
