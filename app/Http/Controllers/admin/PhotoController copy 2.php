...
class PhotoController extends Controller
{
    public function __construct(Request $request){
        ...
        $this->Maincategories = category::where(['parent_id'=>0])->orderBy('item_no')->get();
    ...
    if($sub_category){
        ...
        if($request->page == 'manage' && $request->main_category != null && $request->sub_category != null)
        {
            ...
            if($isProductAvailable){
                $photoDetails = DB::table('media')->orderBy('item_no')->where('media_id', $isProductAvailable->id)->get();
            }else{
                $photoDetails = null;
            }
            $data = [
